...
table, hashmap, hashtable, symboltable, associative-array, object, and others. There are
...
During iteration, you can retrieve the key and value of an element by calling the `first`
and `second` fields of the `Pair` object produced:
...
iterate over the keys, values, or keys and values respectively. Note that elements in a
dictionary are unordered, so loops over dictionaries will likely return elements in a random
order.
...
function dicts()
    ...
    printstyled(header1, '\n', '≡'^(length(header1) + 2), '\n'; bold=true);
    println("""
    @doc    @isdefined    @locals\n""")

    header2 = "Dict Constructors"
    printstyled(header2, '\n', '≡'^(length(header2) + 2), '\n'; bold=true);
    println("""
    Dict    IdDict    WeakKeyDict    ImmutableDict\n""")
    ...
    printstyled(header4, '\n', '≡'^(length(header4) + 2), '\n'; bold=true);
    println("""
    count     findall      findmax    findprev
    filter    findfirst    findmin    first\n""")

    header5 = "Reduce Functions"
    printstyled(header5, '\n', '≡'^(length(header5) + 2), '\n'; bold=true);
    println("""
    ...
    printstyled(header6, '\n', '≡'^(length(header6) + 2), '\n'; bold=true);
    println("""
    copy!      empty!     get!    merge!        pop!     replace!     sizehint!
    delete!    filter!    map!    mergewith!    push!    setindex!\n""")

    header6 = "General Functions"
    printstyled(header6, '\n', '≡'^(length(header6) + 2), '\n'; bold=true);
    println("""
    IteratorEltype    get            merge                  supertypes
    IteratorSize      getindex       mergewith              supertype
    Pairs             getkey         nameof                 summary
    broadcast         hash           objectid               summarysize
    checked_length    iterate        only                   typeassert
    copy              join           pointer_from_objref    typeintersect
    copymutable       keys           rand                   typejoin
    deepcopy          keytype        replace                typeof
    eltype            length         rest                   unique
    dump              map            sizeof                 valtype
    eachindex         methodswith    subtypes               values \n""")

    header7 = "True/False Functions"
    printstyled(header7, '\n', '≡'^(length(header7) + 2), '\n'; bold=true);
    println("""
    all          haskey            isconst       isequal            issorted
    allequal     in                isdefined     isiterable         isstructtype
    allunique    isa               isdisjoint    ismutable          issubset
    any          isabstracttype    isdone        ismutabletype
    hasfastin    isconcretetype    isempty       isprimitivetype\n""")

    header8 = "Type-Conversion Functions"
    printstyled(header8, '\n', '≡'^(length(header8) + 2), '\n'; bold=true);
    println("""
    collect    convert    oftype    string\n""")

    header9 = "Mathematical Functions"
    printstyled(header9, '\n', '≡'^(length(header9) + 2), '\n'; bold=true);
    println("""
    argmax    cumprod    extrema      maximum    prod       sum        union
    argmin    cumsum     intersect    minimum    setdiff    symdiff\n""")

    header10 = "Non-Alphanumeric Operators"
