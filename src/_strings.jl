...
- String literals are delimited by double quotes or triple double quotes. Triple-quoted
  string literals can contain `"` characters without escaping:
...
character because `|` is not a valid continuation to it. Finally, the string s2 contains one
too high code point.
...
concatenation is done with `string`. `*` is also provided for string concatenation:
...
- The dedentation level is determined as the longest common starting sequence of spaces or
  ...
  all lines, excluding the text following the opening \""", the common starting sequence is
  removed (including lines containing only spaces and tabs if they start with this
  sequence), e.g.:
```
julia> \"""    This
         is
...
of 8 adjacent binary digits (bits), each of which consists of a 0 or 1. Originally, a byte
was any string of more than one bit that made up a simple piece of information like a single
character.
...
function strings()
    ...
    printstyled(header1, '\n', '≡'^(length(header1) + 2), '\n'; bold=true);
    println("""
    @b_str      @html_str     @lazy_str    @s_str    @text_str    @view
    @big_str    @isdefined    @raw_str     @simd     @v_str\n""")

    header2 = "Indices Functions"
    printstyled(header2, '\n', '≡'^(length(header2) + 2), '\n'; bold=true);
    println("""
    eachindex     getindex     nextind    reverseind
    firstindex    lastindex    prevind    thisind\n""")
    ...
    header4 = "Case-Sensitive Functions"
    printstyled(header4, '\n', '≡'^(length(header4) + 2), '\n'; bold=true);
    println("""
    lowercase    lowercasefirst    titlecase    uppercase    uppercasefirst\n""")

    header5 = "Splitting & Stripping Functions"
    printstyled(header5, '\n', '≡'^(length(header5) + 2), '\n'; bold=true);
    println("""
    chomp    chopprefix    eachsplit    rsplit    split
    chop     chopsuffix    lstrip       rstrip    strip\n""")
    ...
    printstyled(header7, '\n', '≡'^(length(header7) + 2), '\n'; bold=true);
    println("""
    CodeUnits             eltype               pointer       summary
    Generator             escape_raw_string    prompt        summarysize
    IteratorEltype        escape_string        rand          typeassert
    IteratorSize          indentation          repeat        typeintersect
    SubString             iterate              replace       typejoin
    SubstitutionString    join                 repr          typemin
    alignment             keys                 rest          typeof
    apropos               length               reverse       textwidth
    ascii                 lpad                 rpad          transcode
    broadcast             one                  sizeof        unescape_string
    checked_length        only                 splatf         unindent
    clipboard             methodswith          subtypes      unique
    codeunit              nameof               supertype     view
    codeunits             ncodeunits           supertypes\n""")

    header8 = "True/False Functions"
    printstyled(header8, '\n', '≡'^(length(header8) + 2), '\n'; bold=true);
    println("""
    all            in                isdone            isprimitivetype
    allequal       isa               isempty           issorted
    allunique      isabstracttype    isequal           isstructtype
    any            isbitstype        isgreater         issubset
    checkbounds    isascii           isidentifier      istextmime
    contains       isconcretetype    isiterable        isvalid
    endswith       isconst           isless            occursin
    hasfastin      isdefined         ismutable         startswith
    hastypemax     isdir             ismutabletype     showable
    isabspath      isdisjoint        isone\n""")

    header9 = "Type-Conversion Functions"
    printstyled(header9, '\n', '≡'^(length(header9) + 2), '\n'; bold=true);
    println("""
    collect    convert    hex2bytes    parse    string\n""")

    header10 = "Mathematical Functions"
    printstyled(header10, '\n', '≡'^(length(header10) + 2), '\n'; bold=true);
    println("""
    accumulate    extrema    cumprod      maximum    minmax     symdiff
    argmax        clamp      intersect    min        prod       union
    argmin        cmp        max          minimum    setdiff\n""")
