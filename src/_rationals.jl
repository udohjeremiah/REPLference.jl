...
function rationals()
    ...
    printstyled(header2, '\n', '≡'^(length(header2) + 2), '\n'; bold=true)
    println("""
    bitsunionsize    fieldnames     numerator      subtypes         typemin
    canonicalize2    fieldtype      nameof         supertype        typeof
    denominator      fieldtypes     one            supertypes       zero
    dump             getfield       oneunit        typeassert
    fieldcount       getproperty    sizeof         typeintersect
    fieldindex       methodswith    summary        typejoin
    fieldname        nfields        summarysize    typemax\n""")

    header3 = "True/False Functions"
    printstyled(header3, '\n', '≡'^(length(header3) + 2), '\n'; bold=true)
    println("""
    hasfield          isbitsunion       isgreater        isodd
    hasproperty       isconcretetype    isinf            isone
    hastypemax        isconst           isinteger        isprimitivetype
    isa               isdefined         isless           isreal
    isabstracttype    isequal           ismutable        isstructtype
    isbits            iseven            ismutabletype    iszero
    isbitstype        isfinite          isnan\n""")

    header4 = "Type-Conversion Functions"
    printstyled(header4, '\n', '≡'^(length(header4) + 2), '\n'; bold=true)
    println("""
    big        complex    oftype     rationalize    widen
    convert    float      promote    string\n""")

    header5 = "Mathematical Functions"
    printstyled(header5, '\n', '≡'^(length(header5) + 2), '\n'; bold=true)
    printstyled("Absolute Value, Min/Max & Sign", '\n'; bold=true)
    println("""
    abs     copysign    flipsign    signbit      signless    min
    abs2    cmp         sign        signequal    max         minmax\n""")

    printstyled("Addition, Division & Multiplication", '\n'; bold=true)
    println("""
    cld    fld     fldmod     fma    gcdx    lcm    mod1    muladd      rem
    div    fld1    fldmod1    gcd    inv     mod    modf    nextprod    widemul\n""")

    printstyled("Logs, Powers & Roots", '\n'; bold=true)
    println("""
    cbrt     exp      expm1       hypot     log      log1p      sqrt
    cis      exp2     exponent    ispow2    log2     nextpow
    cispi    exp10    evalpoly    isqrt     log10    prevpow\n""")
    ...
    println("""
    acos     acsc     asin     cos      cotd       rad2deg    sinc        tan
    ...
    acoth    asech    atanh    cot      deg2rad    sind       sinpi\n""")
end