...
- *TYPE EXPRESSIONS* determines how types declarations are to made:
...
The correct way to define such a method are:
```julia
norm(p::Point{<:Real}) = sqrt(p.x^2 + p.y^2)
```
...
arametric abstract type declarations declare a *collection* of abstract types:
...
a "type variable" since it is like a variable that ranges over types. Each where introduces
a single type variable, so these expressions are nested for types with multiple parameters,
...
"""
function types()
    header1 = "General Supertypes"
    printstyled(header1, '\n', '≡'^(length(header1) + 2), '\n'; bold=true)
    println("""
    Any    Bottom    DataType    Type    Union    UnionAll\n""")

    header2 = "Macros"
    printstyled(header2, '\n', '≡'^(length(header2) + 2), '\n'; bold=true)
    println("""
    @atomic        @atomicreplace    @isdefined    @which
    @atomicswap    @doc              @kwdef\n""")

    header3 = "In-Place Operations"
    printstyled(header3, '\n', '≡'^(length(header3) + 2), '\n'; bold=true)
    println("""
    modifyfield!       replacefield!    swapfield!       replaceproperty!
    modifyproperty!    setfield!        swapproperty!    setproperty!\n""")

    header4 = "General Functions"
    printstyled(header4, '\n', '≡'^(length(header4) + 2), '\n'; bold=true)
    println("""
    bitsunionsize              fieldtypes        print            summarysize
    datatype_fielddesc_type    getfield          println          subtypes
    datatype_alignment         getproperty       printstyled      supertype
    dump                       instances         propertynames    supertypes
    fieldcount                 methodswith       real             typeassert
    fieldindex                 nameof            repr             typeof
    fieldname                  nfields           show             typemax
    fieldnames                 nonmissingtype    showarg          typemin
    fieldoffset                one               sizeof           which
    fieldtype                  oneunit           summary          zero\n""")

    header5 = "True/False Functions"
    printstyled(header5, '\n', '≡'^(length(header5) + 2), '\n'; bold=true)
    println("""
    datatype_haspadding     hasmethod         isbitsunion        ismutable
    datatype_nfields        hasproperty       isconst            ismutabletype
    datatype_pointerfree    hastypemax        isconcretetype     isprimitivetype
    has_bottom_parameter    isa               isdefined          issingletontype
    hasfastin               isabstracttype    isdispatchtuple    isstructtype
    hasfield                isbitstype        isiterable\n""")

    header6 = "Type-Conversion & Promotion Functions"
    printstyled(header6, '\n', '≡'^(length(header6) + 2), '\n'; bold=true)
    println("""
    big         oftype          promote_type        typejoin
    complex     parse           promote_typejoin    unsafe_convert
    convert     promote         signed              unsigned
    cconvert    promote_op      tryparse            widen
    float       promote_rule    typeintersect\n""")
end