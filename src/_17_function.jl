...
function functions(; extmod=false)
    ...
    macros = [
        "Macros" => [
            "@__doc__ˣ", "@cfunction", "@code_llvm", "@code_lowered", "@code_native",
            "@code_typed", "@code_warntype", "@functionloc", "@__dot__", "@_inline_metaˣ",
            "@_noinline_metaˣ", "@allocated", "@allocations", "@assert", "@assume_effectsˣ",
            "@atomic", "@atomicreplace", "@atomicswap", "@ccall", "@ccallableˣ",
            "@constpropˣ", "@debug", "@deprecate", "@doc", "@edit", "@elapsed", "@enum",
            "@eval", "@fastmath", "@generated", "@goto", "@inline", "@invoke",
            "@invokelatest", "@label", "@less", "@localsˣ", "@lock", "@lock_nofailˣ",
            "@macroexpand", "@macroexpand1", "@noinline", "@nospecialize", "@polly",
            "@propagate_inboundsˣ", "@pureˣ", "@show", "@showtime", "@specialize",
            "@static", "@task", "@time", "@timed", "@timev", "@which"
        ]
    ]
    methods = [
        "Methods" => [
            "Introspection" => [
                "code_ircodeˣ", "code_lowered", "code_llvm", "code_native", "code_typed",
                "code_warntype", "which"
            ],
            "True/False"    => [
                "applicable", "ifelse", "isa", "isambiguousˣ", "isbits"
            ],
            "Others"        => [
                "accumulate", "all", "any", "argmax", "argmin", "asyncmap", "atexit",
                "atreplinit", "backtrace", "broadcast", "ccall_macro_parseˣ", "cd", "cmp",
                "count", "delete_methodˣ", "donotdeleteˣ", "disable_sigint", "display",
                "dump", "edit", "filter", "finalizer", "findall", "findfirst", "findlast",
                "gen_call_with_extracted_types_and_kwargsˣ", "findmax", "findmin",
                "findnext", "findprev", "foreach", "functionloc", "get", "identity",
                "include_string", "includeˣ", "invoke", "invoke_in_worldˣ", "invokelatest",
                "less", "lock", "lstrip", "macroexpand", "map", "mapfoldl", "mapfoldr",
                "mapreduce", "mapreduce_emptyˣ", "mapreduce_firstˣ", "mapslices", "maximum",
                "may_invoke_generatorˣ", "mergewith", "methods", "methodswith", "minimum",
                "mktemp", "mktempdir", "nameof", "ntuple", "objectid", "open", "precompile",
                "print", "print_statement_costsˣ", "println", "printstyled", "prod",
                "promote_opˣ", "redirect_stderr", "redirect_stdin", "redirect_stdio",
                "redirect_stdout", "redisplay", "reduce", "reenable_sigint", "replace",
                "repr", "retry", "setprecision", "setrounding", "show", "showargˣ",
                "skipchars", "splat", "sprint", "string", "sum", "summary", "typeassert",
                "typeof", "unique", "withenv"
            ]
        ]
    ]
    types = [
        "Types" => [
            "BottomRFˣ", "CFunctionˣ", "ComposedFunction", "DataType", "FilteringRFˣ",
            "Fix1ˣ", "Fix2ˣ", "FlatteningRFˣ", "Function", "Generatorˣ", "MappingRFˣ",
            "Returns", "Splatˣ", "Timer",
        ]
    ]
    operators = [
        "Operators" => [
