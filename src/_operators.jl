...
julia> (2, 3)+      # postfix notation not allowed for this
...
function operators()
    ...
    printstyled(header1, '\n', '≡'^(length(header1) + 2), '\n'; bold=true);
    println("""
    prec_arrow         prec_comparison      prec_decl    prec_power     uni_ops
    prec_assignment    prec_control_flow    prec_pair    quoted_syms    uni_syms\n""")
    ...
    printstyled(header3, '\n', '≡'^(length(header3) + 2), '\n'; bold=true);
    println("""
    isbinaryoperator    ispostfixoperator    is_unary_and_binary_operator
    isoperator          isunaryoperator      is_syntactic_operator\n""")
end