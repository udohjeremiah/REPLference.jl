...
These are cases where "range" comes in as the best alternative. That is, "range" is the best
to use in any situation that requires creating an iterable that follows an order or some
sort of pattern.
...
"""
function ranges()
	header1 = "Range Constructors"
    printstyled(header1, '\n', '≡'^(length(header1) + 2), '\n'; bold=true)
    println("""
    :    OneTo    StepRange    StepRangeLen    LinRange    UnitRange    range\n""")

	header2 = "Macros"
    printstyled(header2, '\n', '≡'^(length(header2) + 2), '\n'; bold=true)
    println("""
    @boundscheck    @inbounds    @isdefined    @simd    @view\n""")

	header3 = "Step & Indices Functions"
    printstyled(header3, '\n', '≡'^(length(header3) + 2), '\n'; bold=true)
    println("""
    RangeStepStyle        axes          getindex     reverseind
	RangeStepIrregular    eachindex     indexin      step
	RangeStepRegular      firstindex    lastindex\n""")
	...
	header6 = "Sort Functions"
    printstyled(header6, '\n', '≡'^(length(header6) + 2), '\n'; bold=true)
    println("""
    insorted        searchsortedfirst    sort
	searchsorted    searchsortedlast     sortperm\n""")

	header7 = "Reduce Functions"
    printstyled(header7, '\n', '≡'^(length(header7) + 2), '\n'; bold=true)
    println("""
    foldl       mapfoldr	       mapreduce_first    reduce_first
	foldr		mapreduce		   reduce			  splat
	mapfoldl    mapreduce_empty    reduce_empty\n""")

	header8 = "Cocatenation Functions"
    printstyled(header8, '\n', '≡'^(length(header8) + 2), '\n'; bold=true)
    println("""
    ...
    printstyled(header9, '\n', '≡'^(length(header9) + 2), '\n'; bold=true)
    println("""
    Generator         fieldcount     keytype        nameof         summary
    IteratorEltype    fieldindex     length         only           summarysize
    IteratorSize      fieldname      nfields        print_range    typeassert
    RangeStepStyle    fieldtype      rand           rest           typeintersect
    broadcast         fieldnames     repeat         similar        typejoin
    canonicalize2     getfield       replace        size           typeof
    checked_length    getproperty    reshape        sizeof         unique
    dump              iterate        revers         subtypes       view
    eltype            join           map            supertype      zero
    empty             keys           methodswith    supertypes\n""")

    header10 = "True/False Functions"
    printstyled(header10, '\n', '≡'^(length(header10) + 2), '\n'; bold=true)
    println("""
    all            hasproperty       isconcretetype    ismutable
    allunique      in                isconst           ismutabletype
    allequal       isa               isdefined         isless
    any            isabstracttype    isdisjoint        isperm
    checkindex     isassigned        isdone            isprimitivetype
    checkbounds    isbits            isempty           issorted
    hasfastin      isbitsunion       isequal           isstructtype
    hasfield       isbitstype        isiterable        issubset\n""")

    header11 = "Type-Conversion Functions"
    printstyled(header11, '\n', '≡'^(length(header11) + 2), '\n'; bold=true)
    println("""
    collect    convert    complex    float    promote    string\n""")

    header12 = "Mathematical Functions"
    printstyled(header12, '\n', '≡'^(length(header12) + 2), '\n'; bold=true)
    println("""
    accumulate    clamp      cumsum       lcm    minmax     nextprod    sum
    argmax        cmp        extrema      max    maximum    prod        symdiff
    argmin        cumprod    intersect    min    minimum    setdiff     union\n""")

    header13 = "Non-Alphanumeric Operators"
