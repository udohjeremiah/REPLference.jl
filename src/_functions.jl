...
called, Julia will compile a version of `f` optimized specifically for Int arguments, which
can then be re-used if `f(1)` or `f(21)` is called again.
...
function functions()
    ...
    printstyled(header1, '\n', '≡'^(length(header1) + 2), '\n'; bold=true);
    println("""
    @__dot__           @constprop    @invokelatest     @propagate_inbounds
    @assert            @deprecate    @isdefined        @pure
    @assume_effects    @doc          @label            @something
    @boundscheck       @fastmath     @locals           @specialize
    @ccall             @generated    @inline           @task
    @ccallable         @goto         @noinline
    @cfunction         @inbounds     @nospecialize
    @coalesce          @invoke       @polly\n""")

    header2 = "Method-Internal Introspection"
    printstyled(header2, '\n', '≡'^(length(header2) + 2), '\n'; bold=true);
    println("""
    @code_llvm       @code_typed       code_lowered    code_typed_by_type
    @code_lowered    @code_warntype    code_native     code_warntype
    @code_native     code_llvm         code_typed\n""")

    header3 = "Method-Introspection Operations"
    printstyled(header3, '\n', '≡'^(length(header3) + 2), '\n'; bold=true);
    println("""
    @edit           @less     apropos    functionloc    methods        which
    @functionloc    @which    edit       less           methodswith\n""")

    header4 = "Measuring Operations"
    printstyled(header4, '\n', '≡'^(length(header4) + 2), '\n'; bold=true);
    println("""
    @allocated    @showtime    @timev    Timer
    @elapsed      @time        @timed    timedwait\n""")

    header5 = "General Functions"
    printstyled(header5, '\n', '≡'^(length(header5) + 2), '\n'; bold=true);
    println("""
    BottomRF            MappingRF     broadcast!        invoke_in_world    sprint
    FilteringRF         Returns       delete_method     invokelatest       summary
    Fix1                Task          disable_sigint    nameof             withenv
    Fix2                alignment     finalizer         precompile
    FlatteningRF        atexit        foreach           retry
    ComposedFunction    atreplinit    include           showarg
    CFunction           broadcast     invoke            splat\n""")

    header6 = "True/False Functions"
    printstyled(header6, '\n', '≡'^(length(header6) + 2), '\n'; bold=true);
    println("""
    applicable    hasmethod    isambiguous    isdefined\n""")
end