...
`const im = Complex(false, true)`. `im` is a global constant of type `Complex{Bool}`, and
calling `im` directly returns the constant.
...
function complexs()
    ...
    printstyled(header3, '\n', '≡'^(length(header3) + 2), '\n'; bold=true);
    println("""
    angle            fieldindex     imag           real           supertypes
    bitsunionsize    fieldname      methodswith    reim           typeassert
    bswap            fieldnames     nameof         sizeof         typeintersect
    canonicalize2    fieldtype      nfields        summary        typejoin
    conj             fieldtypes     one            summarysize    typeof
    dump             getfield       oneunit        subtypes       zero
    fieldcount       getproperty    randn          supertype\n""")

    header4 = "True/False Functions"
    printstyled(header4, '\n', '≡'^(length(header4) + 2), '\n'; bold=true);
    println("""
    hasfield          isbitstype        iseven           isnan              iszero
    hasproperty       isbitsunion       isfinite         isodd
    hastypemax        isconcretetype    isinf            isone
    isa               isconst           isinteger        isprimitivetype
    isabstracttype    isdefined         ismutable        isreal
    isbits            isequal           ismutabletype    isstructtype\n""")

    header5 = "Type-Conversion Functions"
    printstyled(header5, '\n', '≡'^(length(header5) + 2), '\n'; bold=true);
    println("""
    Char    complex    float     parse      rationalize    tryparse
    big     convert    oftype    promote    string         widen\n""")

    header5 = "Mathematical Functions"
    printstyled(header5, '\n', '≡'^(length(header5) + 2), '\n'; bold=true);
    printstyled("Absolute Value & Sign", '\n'; bold=true)
    println("""
    abs    abs2    flipsign    sign\n""")
    ...
    println("""
    cis      ispow2    cispi    log     exp         exp2     log2
    exp10    log1p     expm1    sqrt    evalpoly    hypot    log10\n""")
