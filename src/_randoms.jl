...
"""
function randoms()
    header1 = "Generators (creation and seeding)"
    printstyled(header1, '\n', '≡'^(length(header1) + 2), '\n'; bold=true)
    println("""
    AbstractRNG        Sampler           SamplerSimple    default_rng
    MersenneTwister    SamplerType       TaskLocalRNG     seed!
    RandomDevice       SamplerTrivial    Xoshiro\n""")

    header2 = "Random Generation Functions"
    printstyled(header2, '\n', '≡'^(length(header2) + 2), '\n'; bold=true)
    println("""
    bitrand    rand!      randexp!    randn!
    rand       randexp    randn       randstring\n""")

    header3 = "Subsequences, Permutations & Shuffling Functions"
    printstyled(header3, '\n', '≡'^(length(header3) + 2), '\n'; bold=true)
    println("""
    randcycle     randperm     randsubseq     shuffle
    randcycle!    randperm!    randsubseq!    shuffle!\n""")
end