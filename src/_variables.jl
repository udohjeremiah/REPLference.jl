...
In a program, every variable has a name (identifier), data type, size, and value it is bound
to.
...
```julia
julia> usr, pswd, auth_keys, log_keys, ref_keys = "John Pavard",
                                                  "1234",
                                                  "123csdx12",
                                                  "xzzxwe2s434",
                                                  "12idmne";

julia> user_info = (usr, pswd, auth_keys, log_keys, ref_keys);
...
julia> function outer_scope()
           function inner_scope()
               a, b = x, y # inner scope using outer scope variables
               println(a * b)
           end
           ...
           inner_scope()
           println(x + y)
           println(a / b)  # outer scope cannot use inner scope variables
       end
...
Note that while variable bindings can be read externally, they can only be changed within
the module to which they belong. As an escape hatch, you can always evaluate code inside
that module to modify a variable. This guarantees, in particular, that module bindings
cannot be modified externally by code that never calls `eval`.
