...
Within a module, a variable name can be "reserved" without assigning to it by declaring it
as global, for example, `global x`. This prevents name conflicts for globals initialized
after load time. The syntax `M.x = y` does not work to assign a global in another module;
global assignment is always module-local. For example, attempting to assign to `°` outside
the `Shapes` module throws an error:
...
"""
function modules()
    header1 = "Base Submodules"
    printstyled(header1, '\n', '≡'^(length(header1) + 2), '\n'; bold=true)
    println("""
    Base.Broadcast    Base.Iterators    Base.StackTraces
    Base.Docs         Base.Libc         Base.Sys
    Base.GC           Base.Meta         Base.Threads\n""")

    header2 = "Macros"
    printstyled(header2, '\n', '≡'^(length(header2) + 2), '\n'; bold=true)
    println("""
    @__DIR__       @edit    @isdefined    @time_imports
    @__MODULE__    @eval    @less         @which\n""")

    header3 = "General Functions"
    printstyled(header3, '\n', '≡'^(length(header3) + 2), '\n'; bold=true)
    println("""
    Base.MainInclude.include    edit                  load_path      parentmodule
    Base.include                evalfile              methods        pathof
    __module__                  fullname              methodswith    pkgdir
    __precompile__              hash                  moduleroot     require
    DEPOT_PATH                  include_dependency    nameof         which
    LOAD_PATH                   include_string        names          varinfo
    compilecache                less                  objectid\n""")
